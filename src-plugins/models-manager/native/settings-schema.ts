 ...
 *
 * Vendored from `ui-settings/src/client/schema.ts`. The original is a Cordis
 * `Service` provided by the settings shell and reached through `ctx`; this
 * desktop has no such context (the page is rendered by our own plugin), so the
 * class is instantiated directly by `ProvidersTab` and the `Service` base is
 * dropped. Nothing else about it changes.
 *
 * `Schema` comes from the DSH vendor tree and is bundled here rather than
 * required: it is not a platform module word, and rehydrating/validating a
 * settings draft needs the real implementation, not a stand-in.
 */
 ...
 */
export class SettingsSchemaService {
